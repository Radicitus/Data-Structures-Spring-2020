...
template<typename ElementType>
ElementType &SkipListKey<ElementType>::getElement() {
    return element;
}
    ...
    : SkipListSet{std::make_unique<RandomSkipListLevelTester<ElementType>>()}
{
    ...
    tempTail->down = nullptr;
    tempTail->next = nullptr;
    tempTail->key = new SkipListKey(SkipListKind::PosInf, ElementType{});

    Node * tempHead = new Node;
    tempHead->down = nullptr;
    tempHead->next = tempTail;
    tempHead->key = new SkipListKey(SkipListKind::NegInf, ElementType{});
...
SkipListSet<ElementType>::SkipListSet(const SkipListSet& s)
{
    ...
    nodeToAdd->key = new SkipListKey(SkipListKind::Normal, ElementType{});
    ...
    while (temp != nullptr) {
        ...
        while (tempRow != nullptr) {
            tempRowNext = tempRow->next;
            if (*tempRow->key < *nodeToAdd->key && *nodeToAdd->key < *tempRowNext->key) {
                add(tempRow->key->getElement());
            }
            tempRow = tempRowNext;
        }
        temp = tempDown;
...
SkipListSet<ElementType>& SkipListSet<ElementType>::operator=(SkipListSet&& s) noexcept
{
    ...
    elements = s.elements;
    s.elements = tempElements;

    std::unique_ptr<SkipListLevelTester<ElementType>> tempTester = levelTester->clone();
    levelTester = s.levelTester->clone();
    s.levelTester = tempTester->clone();
...
void SkipListSet<ElementType>::add(const ElementType& element)
{
    ...
    Node * nodeToAdd = new Node;
    nodeToAdd->key = new SkipListKey(SkipListKind::Normal, element);

    int maxLevel = 0;
    while (true) {
        if (!levelTester->shouldOccupyNextLevel(element)) {break;}
        maxLevel++;
    }

    Node * prevLayerNode;
    if (maxLevel > levels) {
        for (int i = 0; i < maxLevel - levels; ++i) {
            Node *newHead = new Node;
            newHead->key = new SkipListKey(SkipListKind::NegInf, ElementType{});
            newHead->next = nullptr;
            newHead->down = nullptr;

            Node *newTail = new Node;
            newTail->key = new SkipListKey(SkipListKind::PosInf, ElementType{});
            newTail->next = nullptr;
            newTail->down = nullptr;

            Node *newLevelNodeToAdd = new Node;
            newLevelNodeToAdd->key = new SkipListKey(SkipListKind::Normal, element);
            newLevelNodeToAdd->next = nullptr;
            newLevelNodeToAdd->down = nullptr;

            newHead->down = head;
            newHead->next = newLevelNodeToAdd;
            newLevelNodeToAdd->next = newTail;
            newTail->down = tail;
            head = newHead;
            tail = newTail;

            levels++;
            prevLayerNode = newLevelNodeToAdd;
        }
        ...
        while (temp != nullptr) {
            ...
            while (tempRow != nullptr) {
                tempRowNext = tempRow->next;
                if (*tempRow->key < *nodeToAdd->key && *nodeToAdd->key < *tempRowNext->key) {
                    Node * rowNodeToAdd = new Node;

                    prevLayerNode->down = rowNodeToAdd;

                    rowNodeToAdd->key = new SkipListKey(SkipListKind::Normal, element);
                    rowNodeToAdd->next = nullptr;
                    rowNodeToAdd->down = nullptr;

                    tempRow->next = rowNodeToAdd;
                    rowNodeToAdd->next = tempRowNext;
                    prevLayerNode = rowNodeToAdd;
                }
                if (*tempRow->key == *nodeToAdd->key) {
                    prevLayerNode->down = tempRow;
                    prevLayerNode = tempRow;
                }
                tempRow = tempRowNext;
            }
            temp = tempDown;
        ...
        while (temp != nullptr) {
            ...
            while (tempRow != nullptr) {
                tempRowNext = tempRow->next;
                if (*tempRow->key < *nodeToAdd->key && *nodeToAdd->key < *tempRowNext->key && currentLevel <= maxLevel) {
                    Node * rowNodeToAdd = new Node;
...
{
    SkipListKey tempKey = SkipListKey(SkipListKind::Normal, element);
...
unsigned int SkipListSet<ElementType>::elementsOnLevel(unsigned int level) const noexcept
{
    ...
    SkipListKey tempKey = SkipListKey(SkipListKind::Normal, ElementType{});

    Node * temp = head;
    while (temp != nullptr && currentLevel >= level) {
        Node * tempDown = temp->down;
        Node * tempRow = temp;
        Node * tempRowNext;
        while (tempRow != nullptr) {
            tempRowNext = tempRow->next;
            if (currentLevel == level && !(*tempRow->key == *head->key) && !(*tempRow->key == *tail->key)) {
                count++;
            }
            tempRow = tempRowNext;
        }
...
bool SkipListSet<ElementType>::isElementOnLevel(const ElementType& element, unsigned int level) const
{
    ...
    while (temp != nullptr) {
        ...
        while (tempRow != nullptr) {
            tempRowNext = tempRow->next;
            if (*tempRow->key == tempKey && currentLevel == level) {
                return true;
            }
            tempRow = tempRowNext;
        }
